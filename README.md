...
- 姓名、Email 必填，電話選填。
- 主辦人可加入最多 20 個自訂欄位，設定必填／選填，並支援文字、單選與多選。
- 活動說明支援純文字或經安全清理的 HTML。
- 可設定報名起訖時間與 1–5000 人的人數上限。
- Firebase 原子寫入避免最後名額同時送出造成超收。
- 報名資料只有六位數密鑰驗證成功後才能讀取。
- 主辦人可調整報名時間、關閉或重新開啟報名、永久刪除活動，並下載 UTF-8 CSV。
- 首頁右下角鑰匙可查看目前 Firebase 活動 ID 與預計刪除時間。
- 管理員解鎖後可查看資料刪除時間與即時剩餘倒數。
...
報名資料包含個人資訊，請只收集活動所需欄位。首頁活動索引會公開 Firebase 活動 ID 與預計刪除時間，但不公開活動內容或報名資料。六位數密鑰適合短期、低風險活動；高敏感資料應改用正式帳號登入及伺服器端嘗試次數限制。
